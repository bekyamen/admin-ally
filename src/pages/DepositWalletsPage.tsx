...
export default function DepositWalletsPage() {
  ...
  const openEdit = (w: DepositWallet) => {
    setEditWallet(w);
    setForm({ coin: w.coin, address: w.address });
    setQrFile(null);
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editWallet) {
        const res = await editDepositWalletAPI(editWallet.coin, form.address || undefined, qrFile || undefined);
        setWallets((prev) => prev.map((w) => w.id === editWallet.id ? res.data : w));
        toast.success(`${editWallet.coin} wallet updated`);
      } else {
  ...
  const handleDelete = async (w: DepositWallet) => {
    if (!confirm(`Delete ${w.coin} wallet?`)) return;
    try {
      await deleteDepositWalletAPI(w.coin);
      setWallets((prev) => prev.filter((x) => x.id !== w.id));
      toast.success(`${w.coin} wallet deleted`);
    } catch (err: any) {
          ...
          {wallets.map((w) => (
            <div key={w.id} className="overflow-hidden rounded-xl border border-border bg-card">
              ...
              <div className="flex items-center justify-between p-4">
                <p className="flex-1 truncate font-mono text-xs text-muted-foreground" title={w.address}>{w.address}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-lg font-bold text-foreground">{editWallet ? `Edit ${editWallet.coin} Wallet` : 'Create Deposit Wallet'}</h2>
